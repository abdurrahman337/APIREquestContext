...
test.describe('Test suite backend V1', () => {
  ...
  test('Test case 03 - Delete Post by ID', async ({ request }) => {
    ...
    expect(getPostsResponse.ok()).toBeTruthy();
    const allPosts = await getPostsResponse.json();
    // RETRIVE TEH ID OF THE LAST ELEMET IN THE ARRAY
    const lastButOnePostID = allPosts[allPosts.length - 2].id; // Access the second-to-last post
    ...
    const deleteElementPostResponse = await request.get(`http://localhost:3000/posts/${lastButOnePostID}`)
